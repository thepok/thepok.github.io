...
API void body_mass(int i,float mass){Body&b=bodies[i];b.mass=maxf(mass,.001f);b.invMass=b.motion==2?1/b.mass:0;b.localI=shapes[b.shape].unitI*b.mass;b.localInvI=inverse(b.localI);updateBody(i);}
...
API int step(float dt){if(!(dt>0&&dt<=.1f))return -1;for(int i=0;i<jCount;i++)for(float &v:joints[i].out)v=0;int sub=int(__builtin_ceilf(dt*120-1e-5f));if(sub<1)sub=1;for(int i=0;i<sub;i++){substep(dt/sub);if(errorCode)return errorCode;}for(int i=0;i<bCount;i++){bodies[i].force={};bodies[i].torque={};}return errorCode;}
