...
static void prepareGlobalWelds(){cn=0;for(int i=0;i<jCount;i++)if(joints[i].active==2)cj[cn++]=i;}
...
static void substep(float dt){tickNo++;touchCount=0;for(int i=0;i<bCount;i++){Body&b=bodies[i];parents[i]=i;sizes[i]=1;rootActive[i]=0;rootStill[i]=1;rootSleep[i]=1e30f;if(!b.exists||!b.added)continue;if(b.motion==2&&b.active){b.v+=(gravity*b.gravityFactor+b.force*b.invMass)*dt;b.w+=mv(b.invI,b.torque)*dt;b.v*=1/(1+b.linearDamping*dt);b.w*=1/(1+b.angularDamping*dt);}updateBody(i);bounds(i);}
 ...
 for(int it=0;it<vIterations;it++){if(it&1){for(int i=jCount-1;i>=0;i--)solveJoint(joints[i],dt);}else for(int i=0;i<jCount;i++)solveJoint(joints[i],dt);for(int i=0;i<cCount;i++)solveContact(contacts[i]);if(it%4==3||it==vIterations-1)globalWelds();}
 ...
 for(int it=0;it<pIterations;it++){for(int i=0;i<cCount;i++)positionContact(contacts[i]);for(int i=0;i<jCount;i++)positionJoint(joints[i]);}
