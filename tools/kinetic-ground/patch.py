...
replace('src/benchmark/suite.ts','shots:this.shots,stats,phases,','shots:this.shots,floorMotionClamps:(this.sim.world.k?.floor_clamps?.()??0)-this.initialFloorClamps,stats,phases,')
replace('src/benchmark/suite.ts','const aggregateChecksPass=finite&&','const groundPass=b.minY>=Math.min(-1,a.minY-1)&&optimized.traces.every((t,i)=>Number.isFinite(t.minY)&&t.minY>=Math.min(-1,reference.traces[i]?.minY-1));\n const aggregateChecksPass=finite&&groundPass&&')
replace('src/benchmark/suite.ts','ground:b.minY>=Math.min(-1,a.minY-1),','ground:groundPass,')
replace('tests/benchmark-contract.mjs',"traces:[{finite:true,meanHeight:1,hash:'a'}]","traces:[{finite:true,meanHeight:1,minY:0,hash:'a'}]")
replace('tests/speed-browser.mjs','assert(live.final.meanHeight<10);',"assert(live.final.meanHeight<10);assert.ok(live.traces.every(t=>t.minY>=-1),'A live-scene fragment crossed the ground');assert.ok(live.final.maxSpeed<25,'Unbounded residual debris velocity');")
